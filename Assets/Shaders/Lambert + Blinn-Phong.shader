Shader "Unlit/Lambert+Blinn-Phong"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings fragmentInput) : SV_TARGET
            {
                ...
                // Diffuse
                half3 diffuseGray = dot(fragmentInput.normalWorldSpace, lightDirection);
                half3 diffuseRGB = _BaseColor.xyz * diffuseGray;

                // Blinn-Phong
                half3 halfDirection = SafeNormalize(viewDirection + lightDirection);
                float specularGray = pow(saturate(dot(fragmentInput.normalWorldSpace, halfDirection)), _SpecularIntensity);

                // 叠加光照颜色
                half3 specularRGB = _BaseColor.xyz * specularGray;
                return half4(specularRGB + diffuseGray, 1);
            }
            ENDHLSL
        }
    }
